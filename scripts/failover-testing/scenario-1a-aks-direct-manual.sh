...
if kubectl get pod pgbench-client-scenario1a -n cnpg-database &>/dev/null; then
  echo "🧹 Cleaning up existing test pod from previous run..."
  kubectl delete pod pgbench-client-scenario1a -n cnpg-database --force --grace-period=0 &>/dev/null || true
  sleep 3
