#!/bin/bash
# Scenario 2a: AKS Pod → PgBouncer Pooler → Manual Failover
# Tests failover behavior through connection pooler with manual promotion
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  Scenario 2a: AKS Pod → PgBouncer Pooler → Manual Failover  ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
if ! kubectl get namespace cnpg-database &>/dev/null; then
  echo "❌ ERROR: Namespace cnpg-database not found."
  exit 1
fi
...
if ! kubectl get svc pg-primary-pooler-rw -n cnpg-database &>/dev/null; then
  echo "❌ ERROR: PgBouncer pooler service not found."
  exit 1
fi
...
echo "✓ PgBouncer service found: pg-primary-pooler-rw"
echo "✓ Password retrieved"
echo ""

# Clean up any existing test pod from previous runs
if kubectl get pod pgbench-client-scenario2a -n cnpg-database &>/dev/null; then
  echo "🧹 Cleaning up existing test pod from previous run..."
  kubectl delete pod pgbench-client-scenario2a -n cnpg-database --force --grace-period=0 &>/dev/null || true
  sleep 3
  echo "✓ Cleanup complete"
  echo ""
fi

# Pre-failover consistency check (via pooler)
...
echo "  PgBouncer Pods: $(kubectl get pods -n cnpg-database -l app=pg-primary-pooler -o jsonpath='{range .items[*]}{.metadata.name}{" "}{end}')"
echo ""
...
# Create pgbench pod with pooler connection
cat > "$OUTPUT_DIR/pgbench-pod.yaml" << 'EOF'
apiVersion: v1
kind: Pod
metadata:
  name: pgbench-client-scenario2a
...
spec:
  ...
  - name: pgbench
    ...
    - |
      ...
      until pg_isready -h pg-primary-pooler-rw -U app -d appdb; do
        sleep 1
      done
      
      echo "Pooler ready. Starting workload..."
      echo "Start time: $(date '+%Y-%m-%d %H:%M:%S')"
      
      # Use simple protocol for PgBouncer (transaction mode)
...
kubectl wait --for=condition=Ready pod/pgbench-client-scenario2a -n cnpg-database --timeout=60s

echo "✓ Pgbench client pod started"
echo "✓ Workload running through PgBouncer (5 minutes total)"
echo ""
...
# Monitor PgBouncer reconnection
echo "Checking PgBouncer reconnection..."
sleep 5
kubectl exec -n cnpg-database deployment/pg-primary-pooler -- \
  psql -h localhost -U app -d pgbouncer -c "SHOW POOLS;" 2>/dev/null || echo "⚠ PgBouncer status unavailable"
echo ""
...
kubectl wait --for=condition=Ready=false pod/pgbench-client-scenario2a -n cnpg-database --timeout=60s 2>/dev/null || true
...
kubectl logs pgbench-client-scenario2a -n cnpg-database > "$OUTPUT_DIR/pgbench-output.log"
...
echo "  Connection Type: PgBouncer Pooler (transaction mode)"
echo ""
...
echo "  3. Check PgBouncer stats: kubectl exec -n cnpg-database deployment/pg-primary-pooler -- psql -h localhost -U app -d pgbouncer -c 'SHOW POOLS;'"
echo "  4. Check cluster status: kubectl cnpg status pg-primary -n cnpg-database"
echo "  5. Cleanup: kubectl delete pod pgbench-client-scenario2a -n cnpg-database"
echo ""
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
  kubectl delete pod pgbench-client-scenario2a -n cnpg-database --force --grace-period=0
